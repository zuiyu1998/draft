...
use crate::{
    frame_graph::{
        Handle, IntoArcAnyTransientResource, PassNode, PipelineContainer, ResourceBoard,
...
impl FrameGraph {
    ...
    pub fn insert(&mut self, key: &str, index: Handle<ResourceNode>) {
        let key = key.into();
    ...
    pub fn pass_node(&mut self, name: &str) -> &mut PassNode {
        let handle = Handle::new(self.pass_nodes.len());
    ...
    pub fn get_pass_node_mut(&mut self, handle: &Handle<PassNode>) -> &mut PassNode {
        &mut self.pass_nodes[handle.index]
    }

    pub fn get_pass_node(&self, handle: &Handle<PassNode>) -> &PassNode {
        &self.pass_nodes[handle.index]
    }

    pub fn get_resource_node_mut(&mut self, handle: &Handle<ResourceNode>) -> &mut ResourceNode {
        &mut self.resource_nodes[handle.index]
    }

    pub fn get_resource_node(&self, handle: &Handle<ResourceNode>) -> &ResourceNode {
        ...
        ResourceType: IntoArcAnyTransientResource,
    {
        ...
        let resource_node_handle = Handle::new(self.resource_nodes.len());
